(function(root) {
	...
	var getDeferreds = {};
	var getAllContactsDeferred = new dfrd.Deferred();
	...
	function ContactService() {
		...
		function get(id) {
			...
			deferred
				.always(function(contactModel) {
					if (contactModel) {
						delete getDeferreds[contactModel.attr('id')];
					}
				});
			return deferred;
		}
		...
		function getBySubAccountId(subAccountId) {
			return new getBySubAccountIdClass(subAccountId)
				.done(function() {
					getAllContactsDeferred.resolve(ContactsModel);
				})
				.fail(function() {
					getAllContactsDeferred.reject( new Error( 'Something went wrong while fetching contacts by subAccountId.' ) );
				});
		}
		...
		function getBySubAccountIdClass(subAccountId) {
			...
			if (contacts) {
				// If we have the already fetched, then return the value.
				deferred.resolve.call(deferred, contacts);
			} else if (getDeferred) {
				// If already made the service call, then just return the deferred,
				deferred = getDeferred;
			} else {
